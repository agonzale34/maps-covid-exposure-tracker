...
def download_maps_timeline(year, month, day, cookie_jar):
    ...
    if r.status_code == 200:
        print("File downloaded successfully from google")
...
def process_exposure_day(date, exposures):
    ...
    if exposure_day is not None:
        ...
        print('Validating exposures for: ', date.strftime('%Y-%m-%d'))
        ...
        for place in places:
            ...
            if point:
                ...
                print('Validating:', place_name, '-', place_address)
                for location in locations:
                    if location['address'].lower() == place_address.lower():
                        ...
                        print(begin, end, begin_place)
                        if begin <= begin_place <= end:
                            possible_exposures.append(location)
                            print('ALERT: possible exposure found on:', place_name)
...
def run():
    ...
    print('Interval:', init_date.strftime('%Y-%m-%d'), final_date.strftime('%Y-%m-%d'))

    exposure_file_path = 'exposures/nova-scotia.json'
    ...
    with open(exposure_file_path, 'r') as read_file:
        ...
        for i in range(days_past):
            ...
            process_exposure_day(date_to_check, exposures_data)
